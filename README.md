...
실사 영상 기반 인터랙티브 드라마(FMV, Full Motion Video)의 기획 → 분기 설계 → 집필을 수행하는 각본 엔진입니다. 선형 시나리오가 아니라, 플레이어 선택에 따라 갈라지는 멀티 루트 로맨스를 설계합니다. 한국 스토리타코(Storytaco)의 상업 성공작 라인업을 레퍼런스로 삼았습니다.


특징


스토리타코 실전 하드 룰 내장 — 6챕터 강제(스팀 환불 정책 대응), 1챕터 무료 데모, 캐릭터별 에피소드 5~6개, 캐릭터별 엔딩 챕터
...
tomlANTHROPIC_API_KEY = "sk-ant-api03-..."
...
버전변경v1.0초기 빌드. STEP 0~7 파이프라인. 스토리타코 하드 룰. 관능적 로맨스 톤 LOCKED. 스팀 정책 게이트. 수집요소 도달성 검증. 각색 모드.
